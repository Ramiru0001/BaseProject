...
CModelA3M model;
//座標
//CVector3D pos(0, 0, 0);		//Cube
CVector3D pos(-41.18f, 12.22f, -16.20f); //Dungeon
//回転
CVector3D rot(0, 0, 0);
//移動ベクトル（落下に利用）
CVector3D vec(0, 0, 0);
...
void MainLoop(void) {
	...
	CVector3D v(0, 0, 0);
	//球とモデルの衝突
	auto tri = stage_col.CollisionCupsel(pos + CVector3D(0, 2.0-rad, 0), pos+CVector3D(0,rad,0),rad);
	//接触した麺の数繰り返す
	for (auto& t : tri) {
		//地面に当たったら落下終わり
