...
void MainLoop(void) {
	...
	P = 0.05;
	SkyModel.Render();
	SkyModel.SetScale(P, P, P);
	LandModel.Render();
	LandModel.SetScale(i/2, i, i/2);
...
void Init(void)
{
	...
	SkyModel = COPY_RESOURCE("sky", CModelObj);
	ADD_RESOURCE("land", CModel::CreateModel("Field/Land/land.obj"));
	LandModel = COPY_RESOURCE("land", CModelObj);
	ADD_RESOURCE("model", CModel::CreateModel("Charactor/Golem/golem.a3m"));
