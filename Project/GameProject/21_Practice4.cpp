#include "Base/Base.h"
#include "Game(P4)/Stage.h"
#include "Game(P4)/Player.h"
#include "Game(P4)/PlayerCamera.h"
#include "Game(P4)/Enemy.h"
...
void Init(void)
{
	...
	ADD_RESOURCE("Girl", CModel::CreateModel("Charactor/GirlModel/GirlModel.a3m"));
	ADD_RESOURCE("Stage", CModel::CreateModel("Field/MyArea/Area.obj", 1, 1, 1));

	//乱数の種を設定
	srand(time(NULL));
	//敵を2体追加
	for (int i = 0; i < 2; i++) {
		Base::Add(new Enemy(CVector3D(Utility::Rand(-8.0f, 8.0f), Utility::Rand(0.0f, 0.5f), Utility::Rand(-8.0f, 8.0f))));
	}
	//ステージを追加
	Base::Add(new Stage());
