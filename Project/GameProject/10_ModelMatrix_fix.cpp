...
void MainLoop(void) {
	...
	if (HOLD(CInput::eButton6))
		rot.y += rot_speed;
	scale = scale + scale * CInput::GetMouseWheel()*0.2f;
	if (scale.x < 1) scale = CVector3D(1, 1, 1);
//モデル行列-------------------------------------
	CMatrix t = CMatrix::MTranselate(pos);	//平行移動行列
	CMatrix r = CMatrix::MRotation(rot);	//回転行列
	CMatrix s = CMatrix::MScale(scale);		//スケーリング行列
...
//親子関係(階層構造)-----------------------------
	//子1
	static float c1_rot_y = DtoR(0);
	c1_rot_y += DtoR(1.0f);
	CMatrix c1_mat = mat * CMatrix::MRotationY(c1_rot_y)*CMatrix::MTranselate(0, 0, 4.0f);
	Utility::DrawSphere(c1_mat, 0.5, CVector4D(1, 0, 0, 1));

	//子2
	static float c2_rot_y = DtoR(0);
	c2_rot_y += DtoR(1.0f);
	CMatrix c2_mat = mat* CMatrix::MTranselate(0, 0, 4.0f) * CMatrix::MRotationY(c2_rot_y) * CMatrix::MTranselate(0, 0, 1.0f);
	Utility::DrawSphere(c2_mat, 0.5, CVector4D(0, 0, 1, 1));

	//孫
	static float c3_rot_y = DtoR(0);
	c3_rot_y += DtoR(1.0f);
	CMatrix c3_mat = c1_mat * CMatrix::MRotationY(c1_rot_y) * CMatrix::MTranselate(0, 0, 2.0f);
...
void Init(void)
{
	...
	model = COPY_RESOURCE("F16", CModelObj);
}
