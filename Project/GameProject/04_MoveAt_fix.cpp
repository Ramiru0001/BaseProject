...
//-----------------------------------------------------------------------
	...
	//回転値から方向ベクトルを計算
	CVector3D dir(CVector3D(sin(rot.y), 0, cos(rot.y)));
	...
	//カメラ注視点
	CVector3D at = pos + CVector3D(0, 1.5, 0);
	//カメラ視点
	CVector3D eye = CVector3D(10, 10, 10);
	//カメラ設定
	CCamera::GetCurrent()->LookAt(eye, at, CVector3D(0, 1, 0));
