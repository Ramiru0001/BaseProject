...
//カメラ回転値
CVector3D cam_rot(DtoR(30), DtoR(-180), 0);
//注視点までの距離
...
void MainLoop(void) {
	...
	if (key_dir.LengthSq() > 0.1) {
		...
		//１８０～ー１８０に納めるため、NormalizeAngleを使っている
		rot.y = Utility::NormalizeAngle(cam_ang + key_ang);
		//方向ベクトル
		CVector3D dir(sin(rot.y), 0, cos(rot.y));
		//倒し具合を求める
		float key_length = min(1.0, key_dir.Length());
		//移動量にスティックの倒し具合を反映
		pos += dir * move_speed * key_length;
		model.ChangeAnimation(1);
		model.SetAnimationSpeed(key_length);
	}
	else {
	...
	ControlCamera(pos + CVector3D(0, 1.5f, 0));
